...
import { Heart, CreditCard, Smartphone, Building2, Shield, CheckCircle, Copy, Mail } from 'lucide-react';
...
const campaigns = [
  { id: 'education-for-all', name: 'Education for Rural Children', image: campaignEducationImage },
  { id: 'women-empowerment', name: 'Women Self-Help Groups', image: campaignWomenImage },
  { id: 'clean-water', name: 'Clean Water Initiative', image: campaignWaterImage },
  { id: 'health-camps', name: 'Rural Health Camps', image: campaignHealthImage },
];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-3 gap-8 lg:gap-12">
            ...
            >
                    ...
                    {campaigns.map((campaign) => (
                      ...
                      >
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-3">
                          <p className="text-white text-xs font-medium leading-tight">{campaign.name}</p>
                        </div>
                        {selectedCampaign === campaign.id && (
              ...
              {/* Amount Selection */}
              <h3 className="text-lg font-semibold text-foreground mb-4">{t('donate.selectAmount')}</h3>
              ...
              {currentAmount && currentAmount >= 500 && currentImpact && (
                ...
                >
                  <p className="text-sm text-muted-foreground">{t('donate.yourDonation')}</p>
                  <p className="text-lg font-semibold text-primary">
                    {t(currentImpact.impactKey)}
                  </p>
                </motion.div>
              )}

              {/* Payment Methods */}
              <h3 className="text-lg font-semibold text-foreground mb-4">{t('donate.paymentMethod')}</h3>
              <div className="space-y-3 mb-8">
                {[
                  { icon: CreditCard, labelKey: 'donate.card' },
                  { icon: Smartphone, labelKey: 'donate.upi' },
                  { icon: Building2, labelKey: 'donate.netBanking' },
                ].map((method) => (
                  <label
                    key={method.labelKey}
                  ...
                  >
                    ...
                    <span className="font-medium text-foreground">{t(method.labelKey)}</span>
                  </label>
                ))}
              </div>
              ...
              <Button variant="cta" size="xl" className="w-full gap-2">
                ...
                {t('donate.completeDonation')} - ₹{(currentAmount || 0).toLocaleString()}
              </Button>
              ...
              <div className="bg-primary/5 p-6 rounded-xl">
                ...
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li className="flex items-start gap-2">
                    ...
                    <span>{t('donate.taxBenefit1')}</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle className="w-4 h-4 text-primary mt-0.5 flex-shrink-0" />
                    <span>{t('donate.taxBenefit2')}</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle className="w-4 h-4 text-primary mt-0.5 flex-shrink-0" />
                    <span>{t('donate.taxBenefit3')}</span>
                  </li>
                </ul>
              </div>
            </motion.div>
